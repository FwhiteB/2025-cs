...
QString CRecord::generate_month_report(int m,bool isEn)
{
    ...
    for(auto i=records.begin();i!=records.end();i++)
    {
        if(i->isIncome) tot_money+=i->amount;
        else tot_money-=i->amount;
        if(i->date.month()==m)
        {
            if(i->isIncome) month_money+=i->amount;
            else month_money-=i->amount;
        }
    }
    QString ret;
    if(!isEn)
...
QString CRecord::generate_year_report(int y,bool isEn)
{
    ...
    for(auto i=records.begin();i!=records.end();i++)
    {
        ...
        if(i->date.year()==y)
        {
            if(i->isIncome) year_money+=i->amount;
            else year_money-=i->amount;
        }
    }
    QString ret;
    if(!isEn)
...
QString CRecord::generate_category_report(QString c,bool isEn)
{
    ...
    for(auto i=records.begin();i!=records.end();i++)
    {
        ...
        if(i->category==c)
        {
            if(i->isIncome) cat_money+=i->amount;
            else cat_money-=i->amount;
        }
    }
    QString ret;
    if(!isEn)
